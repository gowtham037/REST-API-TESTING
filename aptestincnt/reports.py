...
class ReportGenerator:
    ...
    def add_entry(self, url, method, status_code, schema_valid, issues, response_time):
        self.entries.append({
            ...
            "schema_valid": schema_valid,  # True, False, or None for skipped
            "issues": issues,
...
def validate_response(url, method, report, payload=None):
    ...
    if response is None:
        report.add_entry(url, method, 0, False, ["No response received"], response_time)
        return
...
if __name__ == "__main__":
    ...
    try:
        ...
        for i in range(num):
            ...
            method = input("\U0001f501 Method (GET/POST/PUT/DELETE): ").strip().upper()

            if method not in ["GET", "POST", "PUT", "DELETE"]:
                print(f"❌ Invalid HTTP method '{method}'. Skipping this entry.")
                continue

            payload = None
            if method in ["POST", "PUT"]:
                body = input("\U0001f4e6 Enter JSON payload (or leave blank): ").strip()
                if body:
                    try:
